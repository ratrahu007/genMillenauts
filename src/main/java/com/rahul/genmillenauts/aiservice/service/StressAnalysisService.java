...
/*
   FINAL VERSION (FIXED):
   ✔ NO VertexAI.Builder (not supported)
   ✔ Uses correct VertexAI constructor
   ✔ Async-safe
   ✔ ENV-based GCP auth (Railway / Render safe)
*/
@Service
...
public class StressAnalysisService {
    ...
    // Convert stress index to mood label
    ...
    // Update or create daily mood entry
    ...
    private void sendAlerts(User user, int stress, String mood) {
        ...
        for (AlertContact c : contacts) {
            try {
                ...
                log.error("Failed to send alert to {}", c.getPhone(), e);
            }
        }
    }
}
